...
def menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🎁 Referral"), KeyboardButton(text="🎟 Promo")],
...
async def uid(m: Message, state: FSMContext):
    ...
    await m.answer(
        ...
        "🎟 Send a promo code or send <code>SKIP</code> to continue."
    )

@router.message(Buy.promo)
async def apply_promo_and_order(m: Message, state: FSMContext):
    d = await state.get_data()
    p = db.execute("SELECT * FROM products WHERE id=?", (d["pid"],)).fetchone()
    u = get_user(m.from_user)
    if not p or not p["active"] or effective_stock(p) < 1:
    ...
    promo = None
    discount = 0
    if m.text.strip().upper() != "SKIP":
        promo, discount, err = validate_promo(m.text, u["id"], p["price"])
        if err:
            return await m.answer(err + "\nSend another code or SKIP.")
    total = round(max(0, p["price"] - discount), 2)
    if u["balance"] < total:
        await state.clear()
        return await m.answer(
    ...
    # Prefer a real inventory code if this product has one.
    code_row = db.execute(
    ...
    try:
        ...
        if code_row:
            db.execute(
                "UPDATE product_codes SET status='sold',sold_to=?,used_at=? WHERE id=? AND status='available'",
                (u["id"], now_text(), code_row["id"])
            )
            delivered_code = code_row["code"]
            db_stock_decrement = False
        else:
            db.execute(
            ...
            delivered_code = None
            db_stock_decrement = True

        cur = db.execute(
            """INSERT INTO orders
            (user_id,product_id,game_uid,total,discount,promo_code,delivered_code,status)
            VALUES(?,?,?,?,?,?,?,?)""",
            (u["id"], p["id"], d["game_uid"], total, discount,
             promo["code"] if promo else None, delivered_code, "pending")
        )
        oid = cur.lastrowid
    ...
    await m.answer(
        ...
        f"Discount: {discount:.2f} {CURRENCY}\n"
...
async def referral(m: Message):
    ...
    link = f"https://t.me/{(await m.bot.me()).username}?start={u['tg_id']}"
    await m.answer(
        f"🎁 <b>Referral Program</b>\n\n"
        f"👥 Referrals: <b>{refs}</b>\n"
        f"💎 Bonus per referral: <b>{REFERRAL_BONUS:g} {CURRENCY}</b>\n\n"
        f"🔗 Your link:\n<code>{link}</code>"
    )

@router.message(Command("promo"))
@router.message(F.text == "🎟 Promo")
async def promo_help(m: Message):
    await m.answer("🎟 To use a promo code, start a purchase and send the code when asked.")
...
async def admin_promos(c: CallbackQuery):
    if not is_admin(c.from_user.id):
        return await c.answer("Denied", show_alert=True)
    rows = db.execute("SELECT * FROM promocodes ORDER BY id DESC LIMIT 30").fetchall()
    if not rows:
        text = "🎟 No promo codes yet."
    else:
        text = "🎟 <b>Promo Codes</b>\n\n" + "\n".join(
            f"<code>{r['code']}</code> — "
            f"{r['discount_value']:g}{'%' if r['discount_type']=='percent' else CURRENCY} "
            f"({r['used_count']}/{r['max_use'] or '∞'}) "
            f"{'🟢' if r['active'] else '🔴'}"
            for r in rows
        )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Add Promo", callback_data="admin:addpromo")],
        [InlineKeyboardButton(text="⬅️ Admin Panel", callback_data="admin:panel")]
    ])
    await c.answer()
...
async def admin_add_promo_start(c: CallbackQuery, state: FSMContext):
    if not is_admin(c.from_user.id):
        return await c.answer("Denied", show_alert=True)
    await c.answer()
    await state.set_state(AdminAction.add_promo)
    await c.message.answer(
        "🎟 Send:\n"
        "<code>CODE | percent/fixed | discount | max_uses | expiry</code>\n\n"
        "Example:\n<code>SAVE10 | percent | 10 | 100 | 2026-12-31</code>\n"
        "For no expiry use <code>-</code>."
    )

@router.message(AdminAction.add_promo)
async def admin_add_promo(m: Message, state: FSMContext):
    if not is_admin(m.from_user.id):
        return
    parts = [x.strip() for x in m.text.split("|")]
    if len(parts) != 5:
        return await m.answer("❌ Invalid format.")
    code, dtype, value, max_use, expiry = parts
    dtype = dtype.lower()
    if dtype not in {"percent", "fixed"}:
        return await m.answer("❌ Type must be percent or fixed.")
    try:
        value = float(value)
        max_use = int(max_use)
        if value <= 0 or max_use < 0:
            raise ValueError
    except ValueError:
        return await m.answer("❌ Invalid discount/max uses.")
    expiry = None if expiry == "-" else expiry
    try:
        db.execute(
            """INSERT INTO promocodes(code,discount_type,discount_value,max_use,expires_at)
            VALUES(?,?,?,?,?)""",
            (code.upper(), dtype, value, max_use, expiry)
        )
        db.commit()
    except sqlite3.IntegrityError:
        return await m.answer("❌ Promo code already exists.")
    admin_log(m.from_user.id, "add_promo", code.upper())
    await state.clear()
    await m.answer(f"✅ Promo <code>{code.upper()}</code> created.", reply_markup=admin_kb())
