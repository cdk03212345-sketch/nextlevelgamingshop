...
async def product(c: CallbackQuery):
    ...
    code_count = db.execute("SELECT COUNT(*) n FROM product_codes WHERE product_id=? AND status='unused'", (p['id'],)).fetchone()['n']
...
async def deliver_code(bot, order_id, user_tg_id):
    row = db.execute(
        "SELECT o.*, p.name FROM orders o JOIN products p ON p.id=o.product_id WHERE o.id=?",
        (order_id,)
    ).fetchone()
    if not row:
...
async def approve(c: CallbackQuery):
    ...
    db.commit()
    log_admin(c.from_user.id, "approve_order_code_delivery" if has_code_inventory(o["product_id"]) else "approve_order", oid)
...
async def adm_codes(c: CallbackQuery):
    ...
    await c.message.answer(
        ...
        "➕ Add: <code>product_id | code1\ncode2\ncode3</code>\n"
        "📋 View: <code>/codes PRODUCT_ID</code>\n"
...
async def codes_cmd(m: Message):
    ...
    if len(parts) < 2:
        return await m.answer("Use: /codes PRODUCT_ID")
    try:
        pid = int(parts[1])
    except ValueError:
        return await m.answer("❌ Invalid product ID.")
    p = db.execute("SELECT name FROM products WHERE id=?", (pid,)).fetchone()
    if not p:
        return await m.answer("❌ Product not found.")
    rows = db.execute("SELECT code,status,order_id FROM product_codes WHERE product_id=? ORDER BY id DESC LIMIT 50", (pid,)).fetchall()
    if not rows:
        return await m.answer(f"🔑 {p['name']}: no codes.")
    text = [f"🔑 <b>{p['name']}</b>"]
    for r in rows:
...
async def adm_addcodes(c: CallbackQuery, state: FSMContext):
    if not can(c.from_user.id, "manager"):
        return await c.answer("Manager only", show_alert=True)
    await admin_prompt(c, state, "addcodes", "🔑 Send: product_id | code1\ncode2\ncode3")
...
async def admin_action(m: Message, state: FSMContext):
    ...
    try:
        if action == "addcodes":
            parts = text.split("|", 1)
            if len(parts) != 2:
                raise ValueError("Use: product_id | code1\ncode2\ncode3")
            ...
            for code in codes:
                try:
                    db.execute("INSERT INTO product_codes(product_id,code) VALUES(?,?)", (pid, code))
                    added += 1
                except sqlite3.IntegrityError:
                    duplicates += 1
            db.commit()
            log_admin(m.from_user.id, "add_product_codes", f"product={pid},added={added}")
            await m.answer(f"✅ Added {added} code(s).\n⚠️ Duplicate/skipped: {duplicates}")
        elif action == "addp":
