...
order_columns = {r[1] for r in db.execute("PRAGMA table_info(orders)").fetchall()}
if "code_id" not in order_columns:
    db.execute("ALTER TABLE orders ADD COLUMN code_id INTEGER")

db.execute("CREATE INDEX IF NOT EXISTS idx_product_codes_product_status ON product_codes(product_id, status)")
db.execute("CREATE INDEX IF NOT EXISTS idx_product_codes_order ON product_codes(order_id)")
db.commit()
...
def has_code_inventory(product_id):
    return db.execute(
        "SELECT 1 FROM product_codes WHERE product_id=? LIMIT 1",
        (product_id,)
    ).fetchone() is not None

def code_stock(product_id):
    return db.execute(
        "SELECT COUNT(*) n FROM product_codes WHERE product_id=? AND status='unused'",
        (product_id,)
    ).fetchone()["n"]

def sync_code_product_stock(product_id):
    # For code-backed products, the real sellable stock is the number of unused codes.
    if has_code_inventory(product_id):
        n = code_stock(product_id)
        db.execute("UPDATE products SET stock=? WHERE id=?", (n, product_id))
        db.commit()
    return code_stock(product_id)

async def deliver_code_for_order(bot: Bot, oid: int, admin_id: int):
    """Atomically reserve one unused code for the order and deliver it to the buyer."""
    try:
        db.execute("BEGIN IMMEDIATE")
        order = db.execute(
            "SELECT o.*, p.name, u.tg_id AS buyer_tg_id FROM orders o "
        ...
        # If this product has code inventory, one unused code is mandatory.
        if has_code_inventory(order["product_id"]):
            code = db.execute(
            ...
            cur = db.execute(
                "UPDATE product_codes SET status='delivered', order_id=?, user_id=?, delivered_at=CURRENT_TIMESTAMP WHERE id=? AND status='unused'",
                (oid, order["user_id"], code["id"])
            )
            if cur.rowcount != 1:
                db.rollback()
                return False, "race"
            db.execute("UPDATE orders SET status='completed', code_id=? WHERE id=?", (code["id"], oid))
            db.execute("UPDATE products SET stock=(SELECT COUNT(*) FROM product_codes WHERE product_id=? AND status='unused') WHERE id=?", (order["product_id"], order["product_id"]))
            db.commit()
            ...
            await bot.send_message(
                ...
                f"🔑 Your product code:\n<code>{code['code']}</code>\n\n"
                f"⚠️ This code was delivered only once."
            )
            return True, code["code"]

        # Non-code products keep the old approval flow.
        db.execute("UPDATE orders SET status='completed' WHERE id=?", (oid,))
        db.commit()
        return True, None
    except Exception:
        db.rollback()
        logger.exception("Code delivery failed for order %s", oid)
        return False, "error"
...
def admin_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔑 Code Inventory", callback_data="adm:codes"),
...
async def product(c: CallbackQuery):
    ...
    cstock = code_stock(p["id"])
    stock_line = f"🔑 Code stock: {cstock}" if has_code_inventory(p["id"]) else f"📦 Stock: {p['stock']}"
...
async def buy(c: CallbackQuery, state: FSMContext):
    ...
    if has_code_inventory(pid):
        if code_stock(pid) < 1:
            return await c.answer("❌ Product code is out of stock.", show_alert=True)
    elif p["stock"] < 1:
        return await c.answer("Out of stock", show_alert=True)
...
async def create_order(m: Message, state: FSMContext, extra_uid):
    ...
    if has_code_inventory(pid):
        if code_stock(pid) < 1:
            await state.clear()
            return await m.answer("❌ Product code is out of stock.")
    elif p["stock"] < 1:
        await state.clear()
        return await m.answer("❌ Out of stock.")
...
async def approve(c: CallbackQuery):
    ...
    ok, detail = await deliver_code_for_order(c.bot, oid, c.from_user.id)
    if not ok:
        if detail == "no_code":
            await c.answer("❌ No unused code is available for this product.", show_alert=True)
            return await c.message.edit_text(f"⚠️ Order #{oid} is still pending. Add a product code first, then approve again.")
        if detail == "already":
            return await c.answer("Already processed", show_alert=True)
        return await c.answer("❌ Delivery failed. Order remains pending.", show_alert=True)
    log_admin(c.from_user.id, "approve_order_and_deliver_code" if detail else "approve_order", oid)
    await c.answer("Approved & delivered")
    if detail:
        await c.message.edit_text(f"✅ Order #{oid} completed.\n🔑 Product code delivered to the buyer.")
    else:
        await c.message.edit_text(f"✅ Order #{oid} completed.")
...
async def reject(c: CallbackQuery):
    ...
    if has_code_inventory(o["product_id"]):
        db.execute("UPDATE products SET stock=(SELECT COUNT(*) FROM product_codes WHERE product_id=? AND status='unused') WHERE id=?", (o["product_id"], o["product_id"]))
    else:
...
async def adm_codes(c: CallbackQuery):
    ...
    rows = db.execute(
        "SELECT p.id,p.name,COUNT(pc.id) AS total, "
        "SUM(CASE WHEN pc.status='unused' THEN 1 ELSE 0 END) AS unused "
    ...
    if not rows:
        ...
        lines = ["🔑 <b>Code Inventory</b>"]
        for r in rows:
            lines.append(f"🆔 {r['id']} — {r['name']} | Total: {r['total'] or 0} | Unused: {r['unused'] or 0}")
        lines.append("\n➕ /addcode PRODUCT_ID\n📋 /codes PRODUCT_ID\n🗑 /delcode CODE")
        await c.message.answer("\n".join(lines))
    await c.answer()

@router.message(Command("addcode"))
async def addcode(m: Message):
    if not can(m.from_user.id, "manager"):
        return await m.answer("⛔ Manager only.")
    payload = m.text.partition(" ")[2].strip()
    if not payload:
        return await m.answer("➕ Use: /addcode PRODUCT_ID | CODE1 | CODE2 | CODE3")
    try:
        parts = [x.strip() for x in payload.split("|")]
        if len(parts) < 2:
            return await m.answer("➕ Use: /addcode PRODUCT_ID | CODE1 | CODE2 | CODE3")
        product_id = int(parts[0])
        p = db.execute("SELECT id,name FROM products WHERE id=?", (product_id,)).fetchone()
        if not p:
            return await m.answer("❌ Product ID not found.")
        codes = [x for x in parts[1:] if x]
        added = 0
        for code in codes:
            try:
                db.execute("INSERT INTO product_codes(product_id,code) VALUES(?,?)", (product_id, code))
                added += 1
            except sqlite3.IntegrityError:
                pass
        db.commit()
        sync_code_product_stock(product_id)
        log_admin(m.from_user.id, "add_product_codes", f"product={product_id},added={added}")
        await m.answer(f"✅ Added {added} code(s) to <b>{p['name']}</b>.\n🔑 Unused code stock: {code_stock(product_id)}")
    except Exception as e:
        logger.exception("addcode failed")
        await m.answer(f"❌ Error: {e}")

@router.message(Command("codes"))
async def codes(m: Message):
    if not can(m.from_user.id, "manager"):
        return await m.answer("⛔ Manager only.")
    try:
        ...
        product_id = int(arg)
        rows = db.execute(
            "SELECT id,code,status,order_id,user_id,created_at,delivered_at FROM product_codes WHERE product_id=? ORDER BY id",
            (product_id,)
        ).fetchall()
        if not rows:
            return await m.answer("❌ No codes found for this Product ID.")
        lines = [f"🔑 <b>Codes for Product ID {product_id}</b>"]
        for r in rows:
            icon = "🟢" if r["status"] == "unused" else "🔴"
            lines.append(f"{icon} #{r['id']} <code>{r['code']}</code> — {r['status']}" + (f" — Order #{r['order_id']}" if r['order_id'] else ""))
        await m.answer("\n".join(lines))
    except Exception:
        await m.answer("📋 Use: /codes PRODUCT_ID")

@router.message(Command("delcode"))
async def delcode(m: Message):
    if not can(m.from_user.id, "manager"):
        return await m.answer("⛔ Manager only.")
    code_text = m.text.partition(" ")[2].strip()
    if not code_text:
        return await m.answer("🗑 Use: /delcode CODE")
    row = db.execute("SELECT * FROM product_codes WHERE code=?", (code_text,)).fetchone()
    if not row:
        return await m.answer("❌ Code not found.")
    if row["status"] != "unused":
        return await m.answer("❌ Delivered codes cannot be deleted.")
    db.execute("DELETE FROM product_codes WHERE id=?", (row["id"],))
    db.commit()
    sync_code_product_stock(row["product_id"])
    log_admin(m.from_user.id, "delete_unused_code", code_text)
    await m.answer("🗑 Unused code deleted.")
