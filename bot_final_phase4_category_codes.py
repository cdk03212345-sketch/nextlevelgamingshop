...
def has_code_inventory(product_id):
    ...
    category = row["category"] if row else None
    if category:
        if db.execute("SELECT 1 FROM product_codes WHERE category=? LIMIT 1", (category,)).fetchone():
            return True
    return db.execute(
        "SELECT 1 FROM product_codes WHERE product_id=? LIMIT 1", (product_id,)
...
async def deliver_code(bot, order_id, user_tg_id):
    ...
    code = available_code(row['category'])
    if not code:
        return False, "No unused product code is available."
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cur = db.execute(
        "UPDATE product_codes SET status='used', order_id=?, user_id=?, used_at=? WHERE id=? AND status='unused'",
        (order_id, row['user_id'], now, code['id'])
    )
    if cur.rowcount != 1:
        return False, "Code was already claimed. Please try again."
    db.execute("UPDATE orders SET delivery_code=? WHERE id=?", (code['code'], order_id))
    db.commit()
    try:
        ...
        logger.exception("Failed to deliver product code")
...
async def approve(c: CallbackQuery):
    ...
    if has_code_inventory(db.execute("SELECT category FROM products WHERE id=?", (o["product_id"],)).fetchone()["category"]):
        ...
        if not ok:
            return await c.answer("No unused code available. Add a code first.", show_alert=True)
    db.execute("UPDATE orders SET status='completed' WHERE id=?", (oid,))
    db.commit()
    log_admin(c.from_user.id, "approve_order_code_delivery" if has_code_inventory(db.execute("SELECT category FROM products WHERE id=?", (o["product_id"],)).fetchone()["category"]) else "approve_order", oid)
    await c.answer("Approved")
    await c.message.edit_text(f"✅ Order #{oid} completed." + ("\n🔑 Product code delivered to the buyer." if has_code_inventory(o["product_id"]) else ""))
...
async def adm_codes(c: CallbackQuery):
    ...
    await c.message.answer(
        ...
        "➕ Add by category: <code>category | code1\ncode2\ncode3</code>\n"
...
async def adm_addcodes(c: CallbackQuery, state: FSMContext):
    if not can(c.from_user.id, "manager"):
        return await c.answer("Manager only", show_alert=True)
    await admin_prompt(c, state, "addcodes", "🔑 Send: category | code1\ncode2\ncode3\nExample: 161 | UC-001\nUC-002\nUC-003")
...
async def admin_action(m: Message, state: FSMContext):
    ...
    try:
        if action == "addcodes":
            parts = text.split("|", 1)
            if len(parts) != 2:
                raise ValueError("Use: category | code1\ncode2\ncode3\nExample: 161 | UC-001\nUC-002")
            category = parts[0].strip()
            if not category:
                raise ValueError("Category is required")
            # Category must exist on at least one product.
            product = db.execute("SELECT id,name FROM products WHERE category=? ORDER BY id LIMIT 1", (category,)).fetchone()
            if not product:
                raise ValueError(f"No product found in category: {category}")
            ...
            for code in codes:
                try:
                    # product_id is retained for legacy compatibility; category is the real pool key.
                    db.execute("INSERT INTO product_codes(product_id,category,code) VALUES(?,?,?)", (product["id"], category, code))
                    added += 1
                except sqlite3.IntegrityError:
                    duplicates += 1
            db.commit()
            log_admin(m.from_user.id, "add_category_codes", f"category={category},added={added}")
            await m.answer(f"✅ Category <b>{category}</b>: added {added} code(s).\n⚠️ Duplicate/skipped: {duplicates}\n\n📦 One approved order = one code.")
        elif action == "addp":
            ...
            await m.answer(
                ...
                f"📦 Stock: {int(stock)}\n\n"
                f"🔑 Code add করতে এই ID ব্যবহার করো: <code>{product_id} | YOUR-CODE</code>"
            )
        elif action == "editp":
