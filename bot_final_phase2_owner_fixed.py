...
# Owner bootstrap: use Render OWNER_ID when configured.
# If it is missing, bootstrap the known owner account from the bot setup.
# This prevents an old SQLite `staff` row from locking the owner out.
OWNER_ID = int(os.getenv("OWNER_ID", "5275898055") or 5275898055)
ALL_ADMIN_IDS = set(ADMIN_IDS) | ({OWNER_ID} if OWNER_ID else set())
...
# IMPORTANT: UPDATE existing rows too, so an old "staff" role cannot override OWNER_ID.
if OWNER_ID:
    db.execute(
        "INSERT INTO admins(tg_id,role) VALUES(?, 'owner') "
...
for aid in ALL_ADMIN_IDS:
    if aid == OWNER_ID:
        continue
    db.execute(
        "INSERT OR IGNORE INTO admins(tg_id,role) VALUES(?, 'staff')",
        (aid,)
    )
db.commit()
...
def role_of(uid):
    # OWNER_ID always has highest priority, even if the old DB contains "staff".
...
def is_admin(uid):
    return role_of(uid) is not None or uid in ALL_ADMIN_IDS
